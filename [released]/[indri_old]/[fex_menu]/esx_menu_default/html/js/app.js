(function () {
	let MenuTpl =
		'<div id="menu_{{_namespace}}_{{_name}}" style= "overflow: hidden;" class="menu{{#align}} slide-toppe align-{{align}}{{/align}}">' +
			// '<div class="head_{{{css}}}"><span></span></div>' +
			'<div class="head"><span></span></div>' +
			'<div class="categorie"><span>{{{title}}}</span></div>' +
				'<div class="menu-items ripple">' +
					'{{#elements}}' +
						'<div class="menu-item ripple {{#selected}}selected{{/selected}}">' +
	...
	ESX_MENU.open = function (namespace, name, data) {
		...
		// custom post per il lua
		$.post('http://' + ESX_MENU.ResourceName + '/menu_focused', JSON.stringify({ focused: ESX_MENU.getFocused() }));
	}

	ESX_MENU.close = function (namespace, name) {
		if (ESX_MENU.opened[namespace] && ESX_MENU.opened[namespace][name]) {
