...
PawnShopMenu = function()
	...
	local elements = {
		--{ label = "Compra", action = "PawnShop_Buy_Menu" },
		{ label = "Vendi", action = "PawnShop_Sell_Menu" },
	}
...
function OpenSellDialogMenu(itemName, SellPrice)
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'esx_PawnShop_amount_to_sell_dialog', {
		...
		TriggerServerEvent("esx_PawnShop:SellItem",amountToSell,totalSellPrice,itemName, prezzoimport, prezzoimport2)
	end,
	function(data, menu)
		menu.close()	
	end)
